...
async def generate_audio(story, voice):
    ...
    if not azure_creds['key'] or not azure_creds['region']:
        raise ValueError("Azure credentials not found in environment variables")
    
    speech_config = speechsdk.SpeechConfig(
        subscription=azure_creds['key'], 
        region=azure_creds['region']
    )
    
    # Convert edge-tts voice format to Azure format
    # Azure format example: en-US-JennyNeural
    speech_config.speech_synthesis_voice_name = voice
    
    audio_filename = f"./audios/audio-{st.session_state.timestamp}-{voice}.mp3"
    audio_output = speechsdk.AudioConfig(filename=audio_filename)
    ...
    result = synthesizer.speak_text_async(story).get()
...
api_key_source = st.radio("Choose API Keys source:", ("Load from .env", "Enter manually"))

if api_key_source == "Load from .env":
    api_key = load_api_key_from_env()
    azure_creds = load_azure_credentials()
    if not api_key:
        st.error("No OpenAI API key found in .env file. Please enter it manually.")
        api_key = st.text_input("Enter your OpenAI API key:", type="password")
    if not azure_creds['key'] or not azure_creds['region']:
        st.error("Azure credentials not found in .env file. Please enter them manually.")
        azure_key = st.text_input("Enter your Azure Speech key:", type="password")
        azure_region = st.text_input("Enter your Azure region:")
else:
    api_key = st.text_input("Enter your OpenAI API key:", type="password")
    azure_key = st.text_input("Enter your Azure Speech key:", type="password")
    azure_region = st.text_input("Enter your Azure region:")
